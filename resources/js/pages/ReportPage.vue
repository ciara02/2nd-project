              ...
              <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                ...
                <div class="p-4 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent mb-2"> 
                    <div class="flex items-center flex-none w-1/2 max-w-full px-3 mb-2"> 
                      <h6 class="mb-0 dark:text-white">Reports Page</h6> 
                    </div> 
                    <div class="flex flex-wrap -mx-3"> 
                      <div class="flex items-center flex-none w-1/2 max-w-full px-3"> 
                        ...
                        :tickets="tickets"
                        :filters="filters"
                        title="iSupport Ticketing System - Reports"
                        :headers="[
                          ...
                          'Contact Name','Contact Number','Concern', 'Status'
                        ]"
                       :columns="[
                          ...
                          { key: 'status', map: (ticket) => ticket.status || 'N/A' }
                        ]"
                      />
                    </div> 
                     ...
                     <tr v-for="ticket in tickets" :key="ticket.ticket_id" @click="openModal(ticket)" class="table-row-clickable">
                        ...
                        >
                         ...
                         <template v-else-if="col.type === 'badge'">
                          <span
                            :class="[
                              'px-2.5 py-1.4 text-xs rounded-1.8 inline-block whitespace-nowrap font-bold uppercase leading-none text-white text-center',
                              col.badge === 'time' ? timeClass(ticket[col.key]) : statusClass(ticket[col.key])
                            ]"
                            :style="col.badge === 'status' ? { width: '120px', display: 'inline-block', textAlign: 'center' } : {}"
                          >
                            {{ ticket[col.key] }}
                          </span>
                        </template>
                ...
                <BaseModal :isOpen="isModalOpen" @close="closeModal">
                    ...
                    <div class="ticket-details">
                      ...
                      <p class="mt-2"><span class="font-semibold">Status:</span> {{ selectedTicket?.status }}</p>
...
import ExportReportTickets from "../export/ExportTicketsReports.vue";
import axios from 'axios'

export default {
  name: 'ResolveDashboard',
  components: { Sidebar, Navbar, Footer, BaseModal, Multiselect, ExportReportTickets },
  ...
  data() {
    return {
      columns: [
        ...
        { key: 'ticket_completion', label: 'Ticket Completion', type: 'badge',badge: 'time', align: 'center', formatter: (ticket) => ticket.ticket_completion || 'N/A' },
        { key: 'ticket_number', label: 'Ticket#', type: 'text', align: 'center', formatter: (ticket) => ticket.ticket_number || 'N/A' },
        { key: 'assigned_by', label: 'Engineer', type: 'text', align: 'center',  formatter: (ticket) => ticket.engineer_assigned?.assign_name || 'N/A' },
        ...
        { key: 'status', label: 'Status', type: 'badge', align: 'center',badge: 'status', formatter: (ticket) => ticket.status || 'N/A' },
      ],
  ...
  methods: {
   ...
   fetchFiltersData() {
        axios.get('/api/filters/productLine')
        .then(res => {
...
fetchTickets(page = 1) {
  ...
  if (this.filters.status) params.status = this.filters.status.value
  ...
  axios.get('/api/tickets/reports', { params })
    .then(res => {
  ...
  timeClass(ticket_completion) {
  if (!ticket_completion || ticket_completion === 'N/A') {
    return 'na-color';
  }
