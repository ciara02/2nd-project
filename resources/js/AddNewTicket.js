...
export default {
    ...
    return {
      currentYear: new Date().getFullYear(),
        ticketDetailsFields: [
          ...
          { name: 'contact_name', label: 'Contact Name', type: 'text', model: '', required: true},
        ...
        contactFields: [
          ...
          { name: 'productName', label: 'Product Name', type: 'select', model: '', hidden: false, options: [], required: true},
 ...
 methods: {
  ...
  async addTicket() {
    ...
    // ✅ 1. Check required fields in order (only those with required: true)
    for (const field of allFields) {
      if (field.required && (!field.model || field.model === '')) {
        ...
        return; // stop immediately on first missing required field
      }
    }
    ...
    try {
      // ✅ 3. Show loading indicator while saving
      Swal.fire({
        title: 'Processing...',
      ...
      const res = await axios.post('/api/addtickets', formData, {
      ...
      // ✅ 6. Reset all inputs after saving
